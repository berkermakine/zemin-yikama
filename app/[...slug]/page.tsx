...
function getRouteSeo(slug:string[]):RouteSeo|null{
 ...
 if(slug[0]==='yedek-parca'&&slug[1]==='urun'&&slug.length===3){const p=sparePartSeo.find(x=>x.slug===slug[2]);return p?{title:`${p.name} | Zemin Yıkama Makinesi Yedek Parça`,description:p.description,image:p.image}:null}
 return null;
}
...
function pageSchemas(slug:string[],route:RouteSeo){
 ...
 if(slug[0]==='urun'&&slug.length===2){const m=machineSeo.find(x=>x.slug===slug[1]);if(m){const productId=`${url}#product`;const imageId=`${url}#primaryimage`;schemas.push({'@context':'https://schema.org','@type':'WebPage','@id':`${url}#webpage`,url,name:route.title,description:route.description,inLanguage:'tr-TR',primaryImageOfPage:{'@id':imageId},mainEntity:{'@id':productId},isPartOf:{'@id':`${SITE_URL}/#website`}});schemas.push({'@context':'https://schema.org','@type':'Product','@id':productId,name:m.name,alternateName:m.alternateName,description:m.description,image:{'@type':'ImageObject','@id':imageId,url:absolute(m.image),contentUrl:absolute(m.image),caption:m.imageAlt},brand:{'@type':'Brand',name:'Berker Ecoline'},manufacturer:{'@id':`${SITE_URL}/#organization`},model:m.name.replace('Berker ',''),category:`${m.kind} zemin yıkama makinesi`,url,mainEntityOfPage:{'@id':`${url}#webpage`},additionalProperty:m.specs.map(([name,value])=>({'@type':'PropertyValue',name,value}))})}}
 if(slug[0]==='yedek-parca'&&slug[1]==='urun'&&slug.length===3){const p=sparePartSeo.find(x=>x.slug===slug[2]);if(p)schemas.push({'@context':'https://schema.org','@type':'Product',name:p.name,description:p.description,image:absolute(p.image),sku:p.sku,brand:{'@type':'Brand',name:'Berker Makine'},category:'Zemin yıkama makinesi yedek parçası',url,offers:{'@type':'Offer',priceCurrency:'TRY',price:p.price,availability:p.stock>0?'https://schema.org/InStock':'https://schema.org/OutOfStock',url}})}
 return schemas;
}
