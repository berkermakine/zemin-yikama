...
export async function POST(request:Request){
 ...
 else{const fs=await import('node:fs/promises');const path=await import('node:path');const directory=await shopUploadDirectory();await fs.mkdir(directory,{recursive:true});await fs.writeFile(path.join(directory,key),new Uint8Array(bytes))}
